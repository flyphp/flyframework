<?php namespace Fly\View;

use Fly\Support\MessageBag;
use Fly\View\Engines\PhpEngine;
use Fly\Support\ServiceProvider;
use Fly\View\Engines\CompilerEngine;
use Fly\View\Engines\EngineResolver;
use Fly\View\Compilers\SwordCompiler;
...
class ViewServiceProvider extends ServiceProvider {
	...
	public function registerEngineResolver()
	{
		...
		$this->app->bindShared('view.engine.resolver', function($app) use ($me)
		{
			...
			foreach (array('php', 'sword') as $engine)
			{
	 ...
	 *
	 * @param  \Fly\View\Engines\EngineResolver  $resolver
	 * @return void
	 */
	...
	/**
	 * Register the Sword engine implementation.
	 *
	 * @param  \Fly\View\Engines\EngineResolver  $resolver
	 * @return void
	 */
	public function registerSwordEngine($resolver)
	{
		$app = $this->app;

		// The Compiler engine requires an instance of the CompilerInterface, which in
		// this case will be the Sword compiler, so we'll first create the compiler
		// instance to pass into the engine so it can compile the views properly.
		$app->bindShared('sword.compiler', function($app)
		{
			$cache = $app['path.storage'].'/views';

			return new SwordCompiler($app['files'], $cache);
		});

		$resolver->register('sword', function() use ($app)
		{
			return new CompilerEngine($app['sword.compiler'], $app['files']);
		});
	}
	...
	public function registerEnvironment()
	{
		$this->app->bindShared('view', function($app)
		{
			...
			// the various engine implementations such as plain PHP or Sword engine.
