...
class Application extends \Symfony\Component\Console\Application {
	...
	public static function make($app)
	{
		...
		$console = with($console = new static('FlyPHP Framework', $app::VERSION))
								->setFlyphp($app)
								->setExceptionHandler($app['exception'])
								->setAutoExit(false);

		$app->instance('flyconsole', $console);
	...
	{
		require $this->flyphp['path'].'/start/flyconsole.php';

		// If the event dispatcher is set on the application, we will fire an event
		// with the FlyConsole instance to provide each listener the opportunity to
		// register their commands on this application before it gets started.
		if (isset($this->flyphp['events']))
		{
			$this->flyphp['events']
					->fire('flyconsole.start', array($this));
		}
	...
	/**
	 * Run an FlyConsole console command by name.
	 *
	...
	{
		return $this->add($this->flyphp[$command]);
	}
	 ...
	 *
	 * @param  \Fly\Exception\Handler  $handler
	 * @return \Fly\Console\Application
	 */
	 ...
	 * @return \Fly\Console\Application
	 */
	public function setFlyphp($flyphp)
	{
		$this->flyphp = $flyphp;
