...
class RetryCommand extends Command {
	...
	{
		$failed = $this->flyphp['queue.failer']->find($this->argument('id'));

		if ( ! is_null($failed))
		{
			$this->flyphp['queue']->connection($failed->connection)->pushRaw($failed->payload, $failed->queue);

			$this->flyphp['queue.failer']->forget($failed->id);
