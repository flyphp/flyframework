...
class TailCommand extends Command {
	 ...
	 *
	 * @param  string  $connection
	 * @return \Fly\Remote\Connection
	 */
	protected function getRemote($connection)
	{
		return $this->flyphp['remote']->connection($connection);
	}
	...
	protected function getPath($connection)
	{
		...
		if (is_null($connection))
		{
			return base_path().'/app/storage/logs/flyphp.log';
		}
		else
		{
			return $this->getRoot($connection).'/app/storage/logs/flyphp.log';
		}
	}
	...
	{
		return $this->flyphp['config']['remote.connections.'.$connection.'.root'];
	}
