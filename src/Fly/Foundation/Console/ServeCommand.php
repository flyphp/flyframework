...
class ServeCommand extends Command {
	...
	public function fire()
	{
		...
		chdir($this->flyphp['path.base']);

		$host = $this->input->getOption('host');

		$port = $this->input->getOption('port');

		$public = $this->flyphp['path.public'];

		$this->info("FlyPHP development server started on http://{$host}:{$port}");
	...
	protected function getOptions()
	{
		return array(
			...
			array('port', null, InputOption::VALUE_OPTIONAL, 'The port to serve the application on.', 7000),
		);
	}
