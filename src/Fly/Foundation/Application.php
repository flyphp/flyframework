...
use Fly\Config\FileLoader;
use Fly\Container\Container;
use Fly\Filesystem\Filesystem;
use Fly\Support\Facades\Facade;
use Fly\Events\EventServiceProvider;
use Fly\Routing\RoutingServiceProvider;
use Fly\Exception\ExceptionServiceProvider;
use Fly\Config\FileEnvironmentVariablesLoader;
use Symfony\Component\HttpKernel\HttpKernelInterface;
use Symfony\Component\HttpKernel\TerminableInterface;
use Symfony\Component\HttpKernel\Exception\HttpException;
use Symfony\Component\Debug\Exception\FatalErrorException;
use Fly\Support\Contracts\ResponsePreparerInterface;
...
class Application extends Container implements HttpKernelInterface, TerminableInterface, ResponsePreparerInterface {
	...
	protected function registerBaseBindings($request)
	{
		...
		$this->instance('Fly\Container\Container', $this);
	}
	 ...
	 *
	 * @param  \Fly\Support\ServiceProvider|string  $provider
	 * @param  array  $options
	 * @return \Fly\Support\ServiceProvider
	 */
	 ...
	 *
	 * @param  \Fly\Support\ServiceProvider|string  $provider
	 * @return \Fly\Support\ServiceProvider|null
	 */
	 ...
	 *
	 * @param  \Fly\Support\ServiceProvider
	 * @return void
	 */
	...
	protected function getStackedClient()
	{
		...
		$client = with(new \Stack\Builder)
						->push('Fly\Cookie\Guard', $this['encrypter'])
						->push('Fly\Cookie\Queue', $this['cookie'])
						->push('Fly\Session\Middleware', $this['session'], $sessionReject);
	...
	{
		$this->middleware('Fly\Http\FrameGuard');
	}
	 ...
	 * @param  string  $class
	 * @param  array  $parameters
	 * @return \Fly\Foundation\Application
	 */
	...
	public function dispatch(Request $request)
	{
		...
		{
			$response = $this['events']->until('flyphp.app.down');
	...
	{
		$this['events']->listen('flyphp.app.down', $callback);
	}
	 ...
	 *
	 * @return \Fly\Config\LoaderInterface
	 */
	 ...
	 *
	 * @return \Fly\Config\EnvironmentVariablesLoaderInterface
	 */
	 ...
	 *
	 * @return \Fly\Foundation\ProviderRepository
	 */
	...
	{
		$aliases = array(
			'app'            => 'Fly\Foundation\Application',
			'flyconsole'     => 'Fly\Console\Application',
			'auth'           => 'Fly\Auth\AuthManager',
			'auth.reminder.repository' => 'Fly\Auth\Reminders\ReminderRepositoryInterface',
			'sword.compiler' => 'Fly\View\Compilers\SwordCompiler',
			'cache'          => 'Fly\Cache\CacheManager',
			'cache.store'    => 'Fly\Cache\Repository',
			'config'         => 'Fly\Config\Repository',
			'cookie'         => 'Fly\Cookie\CookieJar',
			'encrypter'      => 'Fly\Encryption\Encrypter',
			'db'             => 'Fly\Database\DatabaseManager',
			'events'         => 'Fly\Events\Dispatacher',
			'files'          => 'Fly\Filesystem\Filesystem',
			'form'           => 'Fly\Html\FormBuilder',
			'hash'           => 'Fly\Hashing\HasherInterface',
			'html'           => 'Fly\Html\HtmlBuilder',
			'translator'     => 'Fly\Translation\Translator',
			'log'            => 'Fly\Log\Writer',
			'mailer'         => 'Fly\Mail\Mailer',
			'paginator'      => 'Fly\Pagination\Environment',
			'auth.reminder'  => 'Fly\Auth\Reminders\PasswordBroker',
			'queue'          => 'Fly\Queue\QueueManager',
			'redirect'       => 'Fly\Routing\Redirector',
			'redis'          => 'Fly\Redis\Database',
			'request'        => 'Fly\Http\Request',
			'router'         => 'Fly\Routing\Router',
			'session'        => 'Fly\Session\SessionManager',
			'session.store'  => 'Fly\Session\Store',
			'remote'         => 'Fly\Remote\RemoteManager',
			'url'            => 'Fly\Routing\UrlGenerator',
			'validator'      => 'Fly\Validation\Factory',
			'view'           => 'Fly\View\Environment',
		);
