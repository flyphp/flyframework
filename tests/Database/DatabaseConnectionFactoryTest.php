...
class DatabaseConnectionFactoryTest extends PHPUnit_Framework_TestCase {
	...
	{
		$factory = $this->getMock('Fly\Database\Connectors\ConnectionFactory', array('createConnector', 'createConnection'), array($container = m::mock('Fly\Container\Container')));
	...
	{
		$factory = $this->getMock('Fly\Database\Connectors\ConnectionFactory', array('createConnector'), array($container = m::mock('Fly\Container\Container')));
	...
	{
		$factory = new Fly\Database\Connectors\ConnectionFactory($container = m::mock('Fly\Container\Container'));
		$container->shouldReceive('bound')->andReturn(false);
		$this->assertInstanceOf('Fly\Database\Connectors\MySqlConnector', $factory->createConnector(array('driver' => 'mysql')));
		$this->assertInstanceOf('Fly\Database\Connectors\PostgresConnector', $factory->createConnector(array('driver' => 'pgsql')));
		$this->assertInstanceOf('Fly\Database\Connectors\SQLiteConnector', $factory->createConnector(array('driver' => 'sqlite')));
		$this->assertInstanceOf('Fly\Database\Connectors\SqlServerConnector', $factory->createConnector(array('driver' => 'sqlsrv')));
	}
