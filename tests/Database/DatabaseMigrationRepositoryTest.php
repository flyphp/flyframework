...
class DatabaseMigrationRepositoryTest extends PHPUnit_Framework_TestCase {
	...
	public function testGetRanMigrationsListMigrationsByPackage()
	{
		...
		$connectionMock = m::mock('Fly\Database\Connection');
	...
	{
		$repo = $this->getMock('Fly\Database\Migrations\DatabaseMigrationRepository', array('getLastBatchNumber'), array(
			$resolver = m::mock('Fly\Database\ConnectionResolverInterface'), 'migrations'
		));
	...
	{
		$repo = $this->getMock('Fly\Database\Migrations\DatabaseMigrationRepository', array('getLastBatchNumber'), array(
			m::mock('Fly\Database\ConnectionResolverInterface'), 'migrations'
		));
		$repo->expects($this->once())->method('getLastBatchNumber')->will($this->returnValue(1));
	...
	{
		return new DatabaseMigrationRepository(m::mock('Fly\Database\ConnectionResolverInterface'), 'migrations');
	}
