...
class DatabaseConnectionTest extends PHPUnit_Framework_TestCase {
	...
	public function testFromCreatesNewQueryBuilder()
	{
		...
		$conn->setQueryGrammar(m::mock('Fly\Database\Query\Grammars\Grammar'));
		$conn->setPostProcessor(m::mock('Fly\Database\Query\Processors\Processor'));
		$builder = $conn->table('users');
		$this->assertInstanceOf('Fly\Database\Query\Builder', $builder);
	...
	public function testSchemaBuilderCanBeCreated()
	{
		...
		$this->assertInstanceOf('Fly\Database\Schema\Builder', $schema);
	...
	{
		$connection = $this->getMockConnection();
		$cache  = m::mock('Fly\Cache\CacheManager');
	...
	protected function getMockConnection($methods = array(), $pdo = null)
	{
		...
		return $this->getMock('Fly\Database\Connection', array_merge($defaults, $methods), array($pdo));
	}
