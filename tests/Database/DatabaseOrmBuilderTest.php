...
class DatabaseOrmBuilderTest extends PHPUnit_Framework_TestCase {
	...
	public function testFindMethod()
	{
		...
		$builder = $this->getMock('Fly\Database\Orm\Builder', array('first'), array($query));
	...
	public function testFindWithMany()
	{
		...
		$builder = $this->getMock('Fly\Database\Orm\Builder', array('get'), array($query));
		$model = m::mock('Fly\Database\Orm\Model');
		$model->shouldReceive('getKeyName')->once()->andReturn('foo');
		$model->shouldReceive('getTable')->once()->andReturn('table');
		$query->shouldReceive('from')->once()->with('table');
	...
	{
		$builder = $this->getMock('Fly\Database\Orm\Builder', array('get', 'take'), $this->getMocks());
	...
	{
		$builder = $this->getMock('Fly\Database\Orm\Builder', array('getModels', 'eagerLoadRelations'), $this->getMocks());
		...
		$model = m::mock('Fly\Database\Orm\Model');
		$model->shouldReceive('newCollection')->with(array('bar', 'baz'))->andReturn(new Fly\Database\Orm\Collection(array('bar', 'baz')));
	...
	public function testGetMethodDoesntHydrateEagerRelationsWhenNoResultsAreReturned()
	{
		...
		$model = m::mock('Fly\Database\Orm\Model');
		$model->shouldReceive('newCollection')->with(array())->andReturn(new Fly\Database\Orm\Collection(array()));
		$model->shouldReceive('getTable')->once()->andReturn('foo');
		$builder->getQuery()->shouldReceive('from')->once()->with('foo');
		$builder->setModel($model);
		$results = $builder->get(array('foo'));
	...
	{
		$builder = new Fly\Database\Orm\Builder(new Fly\Database\Query\Builder(
	...
	{
		$builder = $this->getMock('Fly\Database\Orm\Builder', array('loadRelation'), $this->getMocks());
		$nop1 = function() {};
		$nop2 = function() {};
	...
	{
		$builder = $this->getMock('Fly\Database\Orm\Builder', array('getRelation'), $this->getMocks());
		$builder->setEagerLoads(array('orders' => function($query) { $_SERVER['__orm.constrain'] = $query; }));
		...
		$this->assertEquals($relation, $_SERVER['__orm.constrain']);
		unset($_SERVER['__orm.constrain']);
	}
	...
	public function testQueryPassThru()
	{
		...
		$this->assertInstanceOf('Fly\Database\Orm\Builder', $builder->foobar());
	...
	public function testQueryScopes()
	{
		...
		$builder->setModel($model = new OrmBuilderTestScopeStub);
	...
	{
		return m::mock('Fly\Database\Orm\Model');
	}


	protected function getMockOrmBuilder()
	{
		return m::mock('Fly\Database\Orm\Builder');
	}
...
class OrmBuilderTestModelStub extends Fly\Database\Orm\Model {}
class OrmBuilderTestScopeStub extends Fly\Database\Orm\Model {
