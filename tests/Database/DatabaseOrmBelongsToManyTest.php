...
class DatabaseOrmBelongsToManyTest extends PHPUnit_Framework_TestCase {
	...
	public function testModelsAreProperlyHydrated()
	{
		...
		$this->assertInstanceOf('Fly\Database\Orm\Collection', $results);
	...
	public function testModelsAreProperlyMatchedToParents()
	{
		...
		$result1 = new OrmBelongsToManyModelPivotStub;
		$result1->pivot->user_id = 1;
		$result2 = new OrmBelongsToManyModelPivotStub;
		$result2->pivot->user_id = 2;
		$result3 = new OrmBelongsToManyModelPivotStub;
		...
		$model2 = new OrmBelongsToManyModelStub;
		$model2->id = 2;
		$model3 = new OrmBelongsToManyModelStub;
		$model3->id = 3;
	...
	{
		$relation = $this->getMock('Fly\Database\Orm\Relations\BelongsToMany', array('touchIfTouching'), $this->getRelationArguments());
	...
	{
		$relation = $this->getMock('Fly\Database\Orm\Relations\BelongsToMany', array('attach'), $this->getRelationArguments());
	...
	{
		$relation = $this->getMock('Fly\Database\Orm\Relations\BelongsToMany', array('attach', 'detach'), $this->getRelationArguments());
		$query = m::mock('stdClass');
		$query->shouldReceive('from')->once()->with('user_role')->andReturn($query);
		$query->shouldReceive('where')->once()->with('user_id', 1)->andReturn($query);
		$relation->getQuery()->shouldReceive('getQuery')->andReturn($mockQueryBuilder = m::mock('StdClass'));
	...
	{
		$relation = $this->getMock('Fly\Database\Orm\Relations\BelongsToMany', array('attach', 'detach', 'touchIfTouching'), $this->getRelationArguments());
		$query = m::mock('stdClass');
		$query->shouldReceive('from')->once()->with('user_role')->andReturn($query);
		$query->shouldReceive('where')->once()->with('user_id', 1)->andReturn($query);
		$relation->getQuery()->shouldReceive('getQuery')->andReturn($mockQueryBuilder = m::mock('StdClass'));
	...
	{
		$relation = $this->getMock('Fly\Database\Orm\Relations\BelongsToMany', array('touch', 'touchingParent'), $this->getRelationArguments());
	...
	public function getRelationArguments()
	{
		...
		{
			$reflector = new ReflectionClass('Fly\Database\Orm\Relations\Pivot');
...
class OrmBelongsToManyModelStub extends Fly\Database\Orm\Model {
	protected $guarded = array();
}

class OrmBelongsToManyModelPivotStub extends Fly\Database\Orm\Model {
	public $pivot;
	public function __construct()
	{
		$this->pivot = new OrmBelongsToManyPivotStub;
	}
}
