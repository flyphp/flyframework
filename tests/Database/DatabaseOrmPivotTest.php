...
class DatabaseOrmPivotTest extends PHPUnit_Framework_TestCase {
	...
	{
		$parent = m::mock('Fly\Database\Orm\Model[getConnectionName]');
	...
	{
		$parent = m::mock('Fly\Database\Orm\Model[getConnectionName,getDates]');
		$parent->shouldReceive('getConnectionName')->andReturn('connection');
		$parent->shouldReceive('getDates')->andReturn(array());
		$pivot = new DatabaseOrmPivotTestDateStub($parent, array('foo' => 'bar', 'created_at' => 'foo'), 'table');
		$this->assertTrue($pivot->timestamps);

		$pivot = new DatabaseOrmPivotTestDateStub($parent, array('foo' => 'bar'), 'table');
	...
	public function testDeleteMethodDeletesModelByKeys()
	{
		...
		$parent->guard(array());
		$parent->shouldReceive('getConnectionName')->once()->andReturn('connection');
		$pivot = $this->getMock('Fly\Database\Orm\Relations\Pivot', array('newQuery'), array($parent, array('foo' => 'bar'), 'table'));
		$pivot->setPivotKeys('foreign', 'other');
...
class DatabaseOrmPivotTestModelStub extends Fly\Database\Orm\Model {}

class DatabaseOrmPivotTestDateStub extends Fly\Database\Orm\Relations\Pivot {
