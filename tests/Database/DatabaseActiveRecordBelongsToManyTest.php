...
class DatabaseActiveRecordBelongsToManyTest extends PHPUnit_Framework_TestCase {
	...
	{
		$model1 = new ActiveRecordBelongsToManyModelStub;
		$model1->fill(array('name' => 'allan', 'pivot_user_id' => 1, 'pivot_role_id' => 2));
		$model2 = new ActiveRecordBelongsToManyModelStub;
		...
		$this->assertInstanceOf('Fly\Database\ActiveRecord\Collection', $results);
	...
	public function testModelsAreProperlyMatchedToParents()
	{
		...
		$result1 = new ActiveRecordBelongsToManyModelPivotStub;
		$result1->pivot->user_id = 1;
		$result2 = new ActiveRecordBelongsToManyModelPivotStub;
		$result2->pivot->user_id = 2;
		$result3 = new ActiveRecordBelongsToManyModelPivotStub;
		...
		$model2 = new ActiveRecordBelongsToManyModelStub;
		$model2->id = 2;
		$model3 = new ActiveRecordBelongsToManyModelStub;
		$model3->id = 3;
	...
	{
		$relation = $this->getMock('Fly\Database\ActiveRecord\Relations\BelongsToMany', array('touchIfTouching'), $this->getRelationArguments());
	...
	{
		$relation = $this->getMock('Fly\Database\ActiveRecord\Relations\BelongsToMany', array('attach'), $this->getRelationArguments());
	...
	{
		$relation = $this->getMock('Fly\Database\ActiveRecord\Relations\BelongsToMany', array('attach', 'detach'), $this->getRelationArguments());
		$query = m::mock('stdClass');
		$query->shouldReceive('from')->once()->with('user_role')->andReturn($query);
		$query->shouldReceive('where')->once()->with('user_id', 1)->andReturn($query);
		$relation->getQuery()->shouldReceive('getQuery')->andReturn($mockQueryBuilder = m::mock('StdClass'));
	...
	{
		$relation = $this->getMock('Fly\Database\ActiveRecord\Relations\BelongsToMany', array('attach', 'detach', 'touchIfTouching'), $this->getRelationArguments());
		$query = m::mock('stdClass');
		$query->shouldReceive('from')->once()->with('user_role')->andReturn($query);
		$query->shouldReceive('where')->once()->with('user_id', 1)->andReturn($query);
		$relation->getQuery()->shouldReceive('getQuery')->andReturn($mockQueryBuilder = m::mock('StdClass'));
	...
	{
		$relation = $this->getMock('Fly\Database\ActiveRecord\Relations\BelongsToMany', array('touch', 'touchingParent'), $this->getRelationArguments());
	...
	public function getRelationArguments()
	{
		...
		{
			$reflector = new ReflectionClass('Fly\Database\ActiveRecord\Relations\Pivot');
...
class ActiveRecordBelongsToManyModelStub extends Fly\Database\ActiveRecord\Model {
	protected $guarded = array();
}

class ActiveRecordBelongsToManyModelPivotStub extends Fly\Database\ActiveRecord\Model {
	public $pivot;
	public function __construct()
	{
		$this->pivot = new ActiveRecordBelongsToManyPivotStub;
	}
}

class ActiveRecordBelongsToManyPivotStub {
	public $user_id;
}