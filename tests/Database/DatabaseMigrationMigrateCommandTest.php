...
class DatabaseMigrationMigrateCommandTest extends PHPUnit_Framework_TestCase {
	...
	{
		$command = new MigrateCommand($migrator = m::mock('Fly\Database\Migrations\Migrator'), __DIR__.'/vendor');
		$app = array('path' => __DIR__);
		$command->setFlyphp($app);
	...
	{
		$params = array($migrator = m::mock('Fly\Database\Migrations\Migrator'), __DIR__.'/vendor');
		$command = $this->getMock('Fly\Database\Console\Migrations\MigrateCommand', array('call'), $params);
		$app = array('path' => __DIR__);
		$command->setFlyphp($app);
		$migrator->shouldReceive('setConnection')->once()->with(null);
		$migrator->shouldReceive('run')->once()->with(__DIR__.'/database/migrations', false);
