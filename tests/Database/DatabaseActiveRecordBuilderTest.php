...
class DatabaseActiveRecordBuilderTest extends PHPUnit_Framework_TestCase {
	...
	{
		$query = m::mock('Fly\Database\Query\Builder');
		$query->shouldReceive('where')->once()->with('foo', '=', 'bar');
		$builder = $this->getMock('Fly\Database\ActiveRecord\Builder', array('first'), array($query));
	...
	public function testFindWithMany()
	{
		...
		$builder = $this->getMock('Fly\Database\ActiveRecord\Builder', array('get'), array($query));
		$model = m::mock('Fly\Database\ActiveRecord\Model');
		$model->shouldReceive('getKeyName')->once()->andReturn('foo');
		$model->shouldReceive('getTable')->once()->andReturn('table');
		$query->shouldReceive('from')->once()->with('table');
	...
	{
		$builder = $this->getMock('Fly\Database\ActiveRecord\Builder', array('get', 'take'), $this->getMocks());
	...
	{
		$builder = $this->getMock('Fly\Database\ActiveRecord\Builder', array('getModels', 'eagerLoadRelations'), $this->getMocks());
		...
		$model = m::mock('Fly\Database\ActiveRecord\Model');
		$model->shouldReceive('newCollection')->with(array('bar', 'baz'))->andReturn(new Fly\Database\ActiveRecord\Collection(array('bar', 'baz')));
	...
	public function testGetMethodDoesntHydrateEagerRelationsWhenNoResultsAreReturned()
	{
		...
		$model = m::mock('Fly\Database\ActiveRecord\Model');
		$model->shouldReceive('newCollection')->with(array())->andReturn(new Fly\Database\ActiveRecord\Collection(array()));
		$model->shouldReceive('getTable')->once()->andReturn('foo');
		$builder->getQuery()->shouldReceive('from')->once()->with('foo');
		$builder->setModel($model);
		$results = $builder->get(array('foo'));
	...
	{
		$builder = new Fly\Database\ActiveRecord\Builder(new Fly\Database\Query\Builder(
			m::mock('Fly\Database\ConnectionInterface'),
			m::mock('Fly\Database\Query\Grammars\Grammar'),
			m::mock('Fly\Database\Query\Processors\Processor')
		));
		$model = m::mock('Fly\Database\ActiveRecord\Model');
	...
	{
		$builder = $this->getMock('Fly\Database\ActiveRecord\Builder', array('get'), $this->getMocks());
		$model = m::mock('Fly\Database\ActiveRecord\Model');
	...
	public function testPaginateMethodWithGroupedQuery()
	{
		...
		$model->shouldReceive('getTable')->once()->andReturn('foo_table');
		$query = $this->getMock('Fly\Database\Query\Builder', array('from', 'getConnection'), array(
			m::mock('Fly\Database\ConnectionInterface'),
			m::mock('Fly\Database\Query\Grammars\Grammar'),
			m::mock('Fly\Database\Query\Processors\Processor'),
		));
	...
	public function testGetModelsProperlyHydratesModels()
	{
		...
		$model = m::mock('Fly\Database\ActiveRecord\Model[getTable,getConnectionName,newInstance]');
		...
		$model->shouldReceive('newInstance')->andReturnUsing(function() { return new ActiveRecordBuilderTestModelStub; });
	...
	{
		$builder = $this->getMock('Fly\Database\ActiveRecord\Builder', array('loadRelation'), $this->getMocks());
		$nop1 = function() {};
		$nop2 = function() {};
	...
	{
		$builder = $this->getMock('Fly\Database\ActiveRecord\Builder', array('getRelation'), $this->getMocks());
		$builder->setEagerLoads(array('orders' => function($query) { $_SERVER['__active_record.constrain'] = $query; }));
		...
		$this->assertEquals($relation, $_SERVER['__active_record.constrain']);
		unset($_SERVER['__active_record.constrain']);
	}
	...
	public function testQueryPassThru()
	{
		...
		$this->assertInstanceOf('Fly\Database\ActiveRecord\Builder', $builder->foobar());
	...
	public function testQueryScopes()
	{
		...
		$builder->setModel($model = new ActiveRecordBuilderTestScopeStub);
	...
	{
		$nestedQuery = $this->getMockActiveRecordBuilder();
	...
	{
		return new Builder(m::mock('Fly\Database\Query\Builder'));
	}


	protected function getMocks()
	{
		return array(m::mock('Fly\Database\Query\Builder'));
	}

	protected function getMockModel()
	{
		return m::mock('Fly\Database\ActiveRecord\Model');
	}


	protected function getMockActiveRecordBuilder()
	{
		return m::mock('Fly\Database\ActiveRecord\Builder');
	}


	protected function getMockQueryBuilder()
	{
		return m::mock('Fly\Database\Query\Builder');
	}

}

class ActiveRecordBuilderTestModelStub extends Fly\Database\ActiveRecord\Model {}
class ActiveRecordBuilderTestScopeStub extends Fly\Database\ActiveRecord\Model {
