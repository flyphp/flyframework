...
class DatabaseMigratorTest extends PHPUnit_Framework_TestCase {
	...
	{
		$migrator = $this->getMock('Fly\Database\Migrations\Migrator', array('resolve'), array(
			m::mock('Fly\Database\Migrations\MigrationRepositoryInterface'),
			$resolver = m::mock('Fly\Database\ConnectionResolverInterface'),
			m::mock('Fly\Filesystem\Filesystem'),
		));
