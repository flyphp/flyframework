...
use Fly\Database\Query\Expression as Raw;
...
class DatabaseQueryBuilderTest extends PHPUnit_Framework_TestCase {
	...
	{
		$connection = m::mock('Fly\Database\ConnectionInterface');
		$grammar = m::mock('Fly\Database\Query\Grammars\Grammar');
		$processor = m::mock('Fly\Database\Query\Processors\Processor');
		$builder = $this->getMock('Fly\Database\Query\Builder', array('getPaginationCount', 'forPage', 'get'), array($connection, $grammar, $processor));
		$paginator = m::mock('Fly\Pagination\Environment');
	...
	public function testPaginateCorrectlyCreatesPaginatorInstanceForGroupedQuery()
	{
		...
		$grammar = m::mock('Fly\Database\Query\Grammars\Grammar');
		$processor = m::mock('Fly\Database\Query\Processors\Processor');
		$builder = $this->getMock('Fly\Database\Query\Builder', array('get'), array($connection, $grammar, $processor));
		$paginator = m::mock('Fly\Pagination\Environment');
	...
	public function testSQLiteMultipleInserts()
	{
		...
		$builder->getConnection()->shouldReceive('insert')->once()->with('insert into "users" ("email", "name") select ? as "email", ? as "name" union select ? as "email", ? as "name"', array('foo', 'allan', 'bar', 'dayle'))->andReturn(true);
		$result = $builder->from('users')->insert(array(array('email' => 'foo', 'name' => 'allan'), array('email' => 'bar', 'name' => 'dayle')));
		$this->assertTrue($result);
	}
	...
	public function testInsertGetIdMethodRemovesExpressions()
	{
		...
		$result = $builder->from('users')->insertGetId(array('email' => 'foo', 'bar' => new Fly\Database\Query\Expression('bar')), 'id');
		$this->assertEquals(1, $result);
	}
	...
	public function testTruncateMethod()
	{
		...
		$sqlite = new Fly\Database\Query\Grammars\SQLiteGrammar;
		$builder = $this->getBuilder();
		$builder->from('users');
	...
	public function testDynamicWhere()
	{
		...
		$builder    = m::mock('Fly\Database\Query\Builder')->makePartial();
	...
	public function setupCacheTestQuery($cache, $driver)
	{
		...
		$grammar = new Fly\Database\Query\Grammars\Grammar;
		$processor = m::mock('Fly\Database\Query\Processors\Processor');

		$builder = $this->getMock('Fly\Database\Query\Builder', array('getFresh'), array($connection, $grammar, $processor));
	...
	protected function getBuilder()
	{
		$grammar = new Fly\Database\Query\Grammars\Grammar;
		$processor = m::mock('Fly\Database\Query\Processors\Processor');
		return new Builder(m::mock('Fly\Database\ConnectionInterface'), $grammar, $processor);
	}


	protected function getPostgresBuilder()
	{
		$grammar = new Fly\Database\Query\Grammars\PostgresGrammar;
		$processor = m::mock('Fly\Database\Query\Processors\Processor');
		return new Builder(m::mock('Fly\Database\ConnectionInterface'), $grammar, $processor);
	}
	...
	{
		$grammar = new Fly\Database\Query\Grammars\MySqlGrammar;
		$processor = m::mock('Fly\Database\Query\Processors\Processor');
		return new Builder(m::mock('Fly\Database\ConnectionInterface'), $grammar, $processor);
	}
	...
	{
		$grammar = new Fly\Database\Query\Grammars\SQLiteGrammar;
		$processor = m::mock('Fly\Database\Query\Processors\Processor');
		return new Builder(m::mock('Fly\Database\ConnectionInterface'), $grammar, $processor);
	}
	...
	{
		$grammar = new Fly\Database\Query\Grammars\SqlServerGrammar;
		$processor = m::mock('Fly\Database\Query\Processors\Processor');
		return new Builder(m::mock('Fly\Database\ConnectionInterface'), $grammar, $processor);
	}
