...
use Mockery as m;
use Fly\Auth\Reminders\PasswordBroker;
...
class AuthPasswordBrokerTest extends PHPUnit_Framework_TestCase {
	...
	{
		$mocks = $this->getMocks();
		$broker = $this->getMock('Fly\Auth\Reminders\PasswordBroker', array('getUser', 'makeErrorRedirect'), array_values($mocks));
	...
	{
		$broker = $this->getBroker($mocks = $this->getMocks());
		$mocks['users']->shouldReceive('retrieveByCredentials')->once()->with(array('foo'))->andReturn($user = m::mock('Fly\Auth\Reminders\RemindableInterface'));
	...
	public function testBrokerCreatesReminderAndRedirectsWithoutError()
	{
		...
		$mocks = $this->getMocks();
		$broker = $this->getMock('Fly\Auth\Reminders\PasswordBroker', array('sendReminder', 'getUri'), array_values($mocks));
		$mocks['users']->shouldReceive('retrieveByCredentials')->once()->with(array('foo'))->andReturn($user = m::mock('Fly\Auth\Reminders\RemindableInterface'));
	...
	public function testMailerIsCalledWithProperViewTokenAndCallback()
	{
		...
		$user = m::mock('Fly\Auth\Reminders\RemindableInterface');
	...
	public function testRedirectReturnedByRemindWhenPasswordsDontMatch()
	{
		...
		$broker = $this->getBroker($mocks = $this->getMocks());
		$mocks['users']->shouldReceive('retrieveByCredentials')->once()->with($creds)->andReturn($user = m::mock('Fly\Auth\Reminders\RemindableInterface'));
	...
	public function testRedirectReturnedByRemindWhenRecordDoesntExistInTable()
	{
		...
		$broker = $this->getMock('Fly\Auth\Reminders\PasswordBroker', array('validNewPasswords'), array_values($mocks = $this->getMocks()));
		$mocks['users']->shouldReceive('retrieveByCredentials')->once()->with(array_except($creds, array('token')))->andReturn($user = m::mock('Fly\Auth\Reminders\RemindableInterface'));
	...
	{
		unset($_SERVER['__auth.reminder']);
		$broker = $this->getMock('Fly\Auth\Reminders\PasswordBroker', array('validateReset', 'getPassword', 'getToken'), array_values($mocks = $this->getMocks()));
		$broker->expects($this->once())->method('validateReset')->will($this->returnValue($user = m::mock('Fly\Auth\Reminders\RemindableInterface')));
	...
	{
		$mocks = array(
			'reminders' => m::mock('Fly\Auth\Reminders\ReminderRepositoryInterface'),
			'users'     => m::mock('Fly\Auth\UserProviderInterface'),
			'mailer'    => m::mock('Fly\Mail\Mailer'),
			'view'      => 'reminderView',
		);
