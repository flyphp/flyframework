...
class AuthGuardTest extends PHPUnit_Framework_TestCase {
	...
	public function testBasicReturnsNullOnValidAttempt()
	{
		...
		$guard = m::mock('Fly\Auth\Guard[check,attempt]', array($provider, $session));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = m::mock('Fly\Auth\Guard[check]', array($provider, $session));
	...
	{
		$guard = $this->getGuard();
		$guard->setDispatcher($events = m::mock('Fly\Events\Dispatcher'));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = $this->getMock('Fly\Auth\Guard', array('login'), array($provider, $session, $request));
		$guard->setDispatcher($events = m::mock('Fly\Events\Dispatcher'));
		$events->shouldReceive('fire')->once()->with('auth.attempt', array(array('foo'), false, true));
		$user = $this->getMock('Fly\Auth\UserInterface');
	...
	{
		$mock = $this->getGuard();
		$mock->setDispatcher($events = m::mock('Fly\Events\Dispatcher'));
		$events->shouldReceive('fire')->once()->with('auth.attempt', array(array('foo'), false, true));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMock('Fly\Auth\Guard', array('getName'), array($provider, $session, $request));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMock('Fly\Auth\Guard', array('user'), array($provider, $session, $request));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMock('Fly\Auth\Guard', array('getName', 'getRecallerName'), array($provider, $session, $request));
		$mock->setCookieJar($cookies = m::mock('Fly\Cookie\CookieJar'));
		$user = m::mock('Fly\Auth\UserInterface');
		$mock->expects($this->once())->method('getName')->will($this->returnValue('foo'));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$mock = $this->getMock('Fly\Auth\Guard', array('clearUserDataFromStorage'), array($provider, $session, $request));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = new Fly\Auth\Guard($provider, $session, $request);
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		$guard = $this->getMock('Fly\Auth\Guard', array('login', 'user'), array($provider, $session, $request));
		$guard->getSession()->shouldReceive('put')->once()->with($guard->getName(), 10);
		$guard->getProvider()->shouldReceive('retrieveById')->once()->with(10)->andReturn($user = m::mock('Fly\Auth\UserInterface'));
	...
	{
		list($session, $provider, $request, $cookie) = $this->getMocks();
		return new Fly\Auth\Guard($provider, $session, $request);
	}
	...
	{
		return array(
			m::mock('Fly\Session\Store'),
			m::mock('Fly\Auth\UserProviderInterface'),
			Symfony\Component\HttpFoundation\Request::create('/', 'GET'),
			m::mock('Fly\Cookie\CookieJar'),
		);
	}
	...
	{
		return new Fly\Cookie\CookieJar(Request::create('/foo', 'GET'), m::mock('Fly\Encryption\Encrypter'), array('domain' => 'foo.com', 'path' => '/', 'secure' => false, 'httpOnly' => false));
	}
