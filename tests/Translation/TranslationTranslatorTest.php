...
use Mockery as m;
use Fly\Translation\Translator;
...
class TranslationTranslatorTest extends PHPUnit_Framework_TestCase {
	...
	{
		$t = $this->getMock('Fly\Translation\Translator', array('get'), array($this->getLoader(), 'en'));
		...
		$t = $this->getMock('Fly\Translation\Translator', array('get'), array($this->getLoader(), 'en', 'sp'));
	...
	{
		$t = $this->getMock('Fly\Translation\Translator', null, array($this->getLoader(), 'en'));
	...
	public function testGetMethodProperlyLoadsAndRetrievesItemWithLongestReplacementsFirst()
	{
		...
		$this->assertEquals('breeze bar allan', $t->get('foo::bar.baz', array('foo' => 'bar', 'foobar' => 'allan'), 'en'));
		$this->assertEquals('foo', $t->get('foo::bar.foo'));
	}
	...
	{
		return m::mock('Fly\Translation\LoaderInterface');
	}
