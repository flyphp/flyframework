...
class PaginationPaginatorTest extends PHPUnit_Framework_TestCase {
	...
	{
		$p = new Paginator($env = m::mock('Fly\Pagination\Environment'), array('foo', 'bar', 'baz'), 3, 2);
	...
	{
		$p = new Paginator($env = m::mock('Fly\Pagination\Environment'), array('foo', 'bar', 'baz'), 3, 2);
		$this->assertInstanceOf('Fly\Pagination\Environment', $p->getEnvironment());
	}
	...
	{
		$p = new Paginator(m::mock('Fly\Pagination\Environment'), array('a', 'b', 'c'), 3, 2);
		$last = $p->last();
