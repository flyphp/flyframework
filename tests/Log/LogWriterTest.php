...
class LogWriterTest extends PHPUnit_Framework_TestCase {
	...
	{
		$writer = new Writer($monolog = m::mock('Monolog\Logger'), $events = new Fly\Events\Dispatcher);
		$monolog->shouldReceive('addError')->once()->with('foo');

		$events->listen('flyphp.log', function($level, $message, array $context = array())
		{
	...
	{
		$writer = new Writer($monolog = m::mock('Monolog\Logger'), $events = m::mock('Fly\Events\Dispatcher'));

		$callback = function() { return 'success'; };
		$events->shouldReceive('listen')->with('flyphp.log', $callback)->once();
