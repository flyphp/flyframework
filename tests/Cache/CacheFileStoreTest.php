...
class CacheFileStoreTest extends PHPUnit_Framework_TestCase {
	...
	public function testExpiredItemsReturnNull()
	{
		...
		$store = $this->getMock('Fly\Cache\FileStore', array('forget'), array($files, __DIR__));
	...
	{
		$files = $this->mockFilesystem();
		$store = $this->getMock('Fly\Cache\FileStore', array('expiration'), array($files, __DIR__));
	...
	{
		return $this->getMock('Fly\Filesystem\Filesystem');
	}
