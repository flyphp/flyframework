...
class CacheDatabaseStoreTest extends PHPUnit_Framework_TestCase {
	...
	{
		$store = $this->getMock('Fly\Cache\DatabaseStore', array('forget'), $this->getMocks());
		$table = m::mock('StdClass');
		$store->getConnection()->shouldReceive('table')->once()->with('table')->andReturn($table);
		$table->shouldReceive('where')->once()->with('key', '=', 'prefixfoo')->andReturn($table);
	...
	{
		$store = $this->getMock('Fly\Cache\DatabaseStore', array('getTime'), $this->getMocks());
		$table = m::mock('StdClass');
		$store->getConnection()->shouldReceive('table')->once()->with('table')->andReturn($table);
	...
	{
		$store = $this->getMock('Fly\Cache\DatabaseStore', array('put'), $this->getMocks());
	...
	{
		return new DatabaseStore(m::mock('Fly\Database\Connection'), m::mock('Fly\Encryption\Encrypter'), 'table', 'prefix');
	}


	protected function getMocks()
	{
		return array(m::mock('Fly\Database\Connection'), m::mock('Fly\Encryption\Encrypter'), 'table', 'prefix');
	}
