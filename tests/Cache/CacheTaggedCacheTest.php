...
class CacheTaggedCacheTest extends PHPUnit_Framework_TestCase {
	...
	{
		$store = m::mock('Fly\Cache\StoreInterface');
		$tagSet = m::mock('Fly\Cache\TagSet', array($store, array('foo', 'bar')));
		$tagSet->shouldReceive('getNamespace')->andReturn('foo|bar');
		$redis = new Fly\Cache\RedisTaggedCache($store, $tagSet);
