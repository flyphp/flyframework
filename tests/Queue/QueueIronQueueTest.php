...
class QueueIronQueueTest extends PHPUnit_Framework_TestCase {
	...
	{
		$queue = new Fly\Queue\IronQueue($iron = m::mock('IronMQ'), $crypt = m::mock('Fly\Encryption\Encrypter'), m::mock('Fly\Http\Request'), 'default');
	...
	public function testPushProperlyPushesJobOntoIronWithClosures()
	{
		$queue = new Fly\Queue\IronQueue($iron = m::mock('IronMQ'), $crypt = m::mock('Fly\Encryption\Encrypter'), m::mock('Fly\Http\Request'), 'default');
		$name = 'Foo';
		$closure = new Fly\Support\SerializableClosure($innerClosure = function() use ($name) { return $name; });
		$crypt->shouldReceive('encrypt')->once()->with(json_encode(array(
			'job' => 'FlyQueueClosure', 'data' => array('closure' => serialize($closure)), 'attempts' => 1, 'queue' => 'default'
		)))->andReturn('encrypted');
		$iron->shouldReceive('postMessage')->once()->with('default', 'encrypted', array())->andReturn((object) array('id' => 1));
	...
	public function testDelayedPushProperlyPushesJobOntoIronWithTimestamp()
	{
		...
		$queue = $this->getMock('Fly\Queue\IronQueue', array('getTime'), array($iron = m::mock('IronMQ'), $crypt = m::mock('Fly\Encryption\Encrypter'), m::mock('Fly\Http\Request'), 'default'));
	...
	{
		$queue = new Fly\Queue\IronQueue($iron = m::mock('IronMQ'), $crypt = m::mock('Fly\Encryption\Encrypter'), m::mock('Fly\Http\Request'), 'default');
		$queue->setContainer(m::mock('Fly\Container\Container'));
		...
		$this->assertInstanceOf('Fly\Queue\Jobs\IronJob', $result);
	}


	public function testPushedJobsCanBeMarshaled()
	{
		$queue = $this->getMock('Fly\Queue\IronQueue', array('createPushedIronJob'), array($iron = m::mock('IronMQ'), $crypt = m::mock('Fly\Encryption\Encrypter'), $request = m::mock('Fly\Http\Request'), 'default'));
		...
		$this->assertInstanceOf('Fly\Http\Response', $response);
