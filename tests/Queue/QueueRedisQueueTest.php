...
class QueueRedisQueueTest extends PHPUnit_Framework_TestCase {
	...
	{
		$queue = $this->getMock('Fly\Queue\RedisQueue', array('getRandomId'), array($redis = m::mock('Fly\Redis\Database'), 'default'));
	...
	{
		$queue = $this->getMock('Fly\Queue\RedisQueue', array('getTime', 'getRandomId'), array($redis = m::mock('Fly\Redis\Database'), 'default'));
		$queue->expects($this->once())->method('getRandomId')->will($this->returnValue('foo'));
	...
	{
		$queue = $this->getMock('Fly\Queue\RedisQueue', array('getTime', 'migrateAllExpiredJobs'), array($redis = m::mock('Fly\Redis\Database'), 'default'));
		...
		$this->assertInstanceOf('Fly\Queue\Jobs\RedisJob', $result);
	}


	public function testReleaseMethod()
	{
		$queue = $this->getMock('Fly\Queue\RedisQueue', array('getTime'), array($redis = m::mock('Fly\Redis\Database'), 'default'));
		$queue->expects($this->once())->method('getTime')->will($this->returnValue(1));
