...
class QueueListenerTest extends PHPUnit_Framework_TestCase {
	...
	public function testRunProcessCallsProcess()
	{
		...
		$listener = m::mock('Fly\Queue\Listener')->makePartial();
	...
	{
		$listener = new Fly\Queue\Listener(__DIR__);
		...
		$this->assertEquals('php fly queue:work connection --queue="queue" --delay=1 --memory=2 --sleep=3 --tries=0', $process->getCommandLine());
	}
