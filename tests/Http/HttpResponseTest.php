...
class HttpResponseTest extends PHPUnit_Framework_TestCase {
	...
	{
        $response = new Fly\Http\Response(new JsonableStub);
        $this->assertEquals('foo', $response->getContent());
        $this->assertEquals('application/json', $response->headers->get('Content-Type'));

        $response = new Fly\Http\Response();
	...
	{
		$mock = m::mock('Fly\Support\Contracts\RenderableInterface');
		$mock->shouldReceive('render')->once()->andReturn('foo');
		$response = new Fly\Http\Response($mock);
		$this->assertEquals('foo', $response->getContent());
	}
    ...
    {
        $response = new RedirectResponse('foo.bar');
        $response->setRequest(Request::create('/', 'GET', array('name' => 'Allan', 'age' => 26)));
        $response->setSession($session = m::mock('Fly\Session\Store'));
    ...
    public function testInputOnRedirect()
    {
        ...
        $response->setRequest(Request::create('/', 'GET', array('name' => 'Allan', 'age' => 26)));
        $response->setSession($session = m::mock('Fly\Session\Store'));
        $session->shouldReceive('flashInput')->once()->with(array('name' => 'Allan', 'age' => 26));
        $response->withInput();
    }
    ...
    public function testOnlyInputOnRedirect()
    {
        ...
        $response->setRequest(Request::create('/', 'GET', array('name' => 'Allan', 'age' => 26)));
        $response->setSession($session = m::mock('Fly\Session\Store'));
        $session->shouldReceive('flashInput')->once()->with(array('name' => 'Allan'));
    ...
    public function testFlashingErrorsOnRedirect()
    {
        ...
        $provider = m::mock('Fly\Support\Contracts\MessageProviderInterface');
    ...
    public function testRedirectWithErrorsArrayConvertsToMessageBag()
    {
        ...
        $response->setRequest(Request::create('/', 'GET', array('name' => 'Allan', 'age' => 26)));
        $response->setSession($session = m::mock('Fly\Session\Store'));
        $session->shouldReceive('flash')->once()->with('errors', m::type('Fly\Support\MessageBag'));
