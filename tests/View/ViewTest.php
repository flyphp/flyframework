...
class ViewTest extends PHPUnit_Framework_TestCase {
	...
	{
		$view = new View(m::mock('Fly\View\Environment'), m::mock('Fly\View\Engines\EngineInterface'), 'view', 'path', array());
		$view->with('foo', 'bar');
	...
	{
		$view = m::mock('Fly\View\View[render]', array(
				m::mock('Fly\View\Environment'),
				m::mock('Fly\View\Engines\EngineInterface'),
				'view',
				'path',
				array()
		));
	...
	public function testViewNestBindsASubView()
	{
		...
		$this->assertInstanceOf('Fly\View\View', $result);
	}


	public function testViewAcceptsArrayableImplementations()
	{
		$arrayable = m::mock('Fly\Support\Contracts\ArrayableInterface');
	...
	public function testViewGatherDataWithRenderable()
	{
		...
		$view->renderable = m::mock('Fly\Support\Contracts\RenderableInterface');
	...
	public function testWithErrors()
	{
		...
		$this->assertTrue($view->errors instanceof \Fly\Support\MessageBag);
		$foo = $view->errors->get('foo');
		$this->assertEquals($foo[0], 'bar');
		$qu = $view->errors->get('qu');
		$this->assertEquals($qu[0], 'ux');
		$data = array('foo'=>'baz');
		$this->assertTrue($view->withErrors(new \Fly\Support\MessageBag($data)) === $view);
		$foo = $view->errors->get('foo');
