...
class ViewEnvironmentTest extends PHPUnit_Framework_TestCase {
	...
	public function testMakeCreatesNewViewInstanceWithProperPathAndEngine()
	{
		...
		$env->getEngineResolver()->shouldReceive('resolve')->once()->with('php')->andReturn($engine = m::mock('Fly\View\Engines\EngineInterface'));
		$env->getFinder()->shouldReceive('addExtension')->once()->with('php');
		$env->setDispatcher(new Fly\Events\Dispatcher);
	...
	{
		$env = m::mock('Fly\View\Environment[make]', $this->getEnvironmentArgs());
	...
	public function testEnvironmentAddsExtensionWithCustomResolver()
	{
		...
		$environment->getEngineResolver()->shouldReceive('resolve')->once()->with('bar')->andReturn($engine = m::mock('Fly\View\Engines\EngineInterface'));
	...
	public function testClassCallbacks()
	{
		$env = $this->getEnvironment();
		$env->getDispatcher()->shouldReceive('listen')->once()->with('composing: foo', m::type('Closure'));
		$env->setContainer($container = m::mock('Fly\Container\Container'));
	...
	protected function getEnvironment()
	{
		return new Environment(
			m::mock('Fly\View\Engines\EngineResolver'),
			m::mock('Fly\View\ViewFinderInterface'),
			m::mock('Fly\Events\Dispatcher')
		);
	}
