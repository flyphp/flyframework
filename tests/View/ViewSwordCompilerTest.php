...
class ViewSwordCompilerTest extends PHPUnit_Framework_TestCase {
	...
	{
		$compiler = new SwordCompiler($files = $this->getFiles(), __DIR__);
	...
	{
		$compiler = new SwordCompiler($files = $this->getFiles(), null);
	...
	{
		$compiler = new SwordCompiler($this->getFiles(), __DIR__);
...
<?php endif; ?>';
	...
	public function testStatementThatContainsNonConsecutiveParanthesisAreCompiled()
	{
		...
		$expected = "Foo <?php echo \Fly\Support\Facades\Lang::get(function_call('foo(blah)')); ?> bar";
		$this->assertEquals($expected, $compiler->compileString($string));
	}
	...
	{
		$compiler = new SwordCompiler($this->getFiles(), __DIR__);
		$this->assertEquals('<?php echo \Fly\Support\Facades\Lang::get(\'foo\'); ?>', $compiler->compileString("@lang('foo')"));
		$this->assertEquals('<?php echo \Fly\Support\Facades\Lang::choice(\'foo\', 1); ?>', $compiler->compileString("@choice('foo', 1)"));
	}
	...
	{
		return m::mock('Fly\Filesystem\Filesystem');
	}
